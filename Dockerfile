...
WORKDIR /app
# pg_dump for the scheduled backup job (15g). python:3.12-slim is Debian 13
# (trixie), whose native PostgreSQL major is 17 -- so the distro client is
# already new enough (pg_dump must be >= the server's major; client 17 dumps
# any server <= 17). Revisit only if Render moves the DB to 18+.
RUN apt-get update \
